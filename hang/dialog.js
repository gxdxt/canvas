...
export class Dialog{
    ...
    animate(ctx){
        ...
        this.pos.add(move); // 이 부분이 약간 부드러운 움직임 만드는 코드들
    ...
    swingDrag(ctx) {
        ...
        const speed = Math.min(Math.max(speedX, 0), 1); //속도 점점 줄어들게 하는 듯

        let rotation = (MAX_ANGLE / 1) * speed; //나누기 1은 뭐하는거야?
    ...
    down(point){
        if (point.collide(this.pos, WIDTH, HEIGHT)){
            ...
            this.origin.y = HEIGHT * this.mousePos.y / HEIGHT; // 이건 무슨 의미지???
